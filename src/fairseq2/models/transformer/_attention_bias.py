...
class AttentionBias(ABC):
    ...
    def materialize(
        ...
        if q_layout.is_packed ^ k_layout.is_packed:
            raise ValueError(
                "`q_layout` and `k_layout` must be both packed or non-packed."
            )

        q_seq_ranges = q_layout.get_seq_ranges()
        k_seq_ranges = k_layout.get_seq_ranges()

        if len(q_seq_ranges) != len(k_seq_ranges):
            raise ValueError(
                f"The number of sequence ranges in `q_layout` and `k_layout` must match, but they are {len(q_seq_ranges)} and {len(k_seq_ranges)} instead."
            )

        q_len = q_layout.shape[1]
        k_len = k_layout.shape[1]

        if q_layout.is_packed:
            # ([H], S, S_kv)
            return self._create_block_bias_tensor(
                q_seq_ranges, q_len, k_seq_ranges, k_len, device, dtype
            )

        batch_size = len(q_seq_ranges)

        if batch_size == 0:
            return torch.zeros((), device=device, dtype=dtype)

        first_q_seq_range = q_seq_ranges[0]
        first_k_seq_range = k_seq_ranges[0]

        if batch_size > 1:
            uniform_q_ranges = all(r == first_q_seq_range for r in q_seq_ranges[1:])
            uniform_k_ranges = all(r == first_k_seq_range for r in k_seq_ranges[1:])

            if not uniform_q_ranges or not uniform_k_ranges:
                biases = []

                for idx in range(batch_size):
                    q_seq_range = q_seq_ranges[idx]
                    k_seq_range = k_seq_ranges[idx]

                    # ([H], S, S_kv)
                    bias = self._create_block_bias_tensor(
                        [q_seq_range], q_len, [k_seq_range], k_len, device, dtype
                    )
        ...
        return self._create_block_bias_tensor(
            [first_q_seq_range], q_len, [first_k_seq_range], k_len, device, dtype
        )

    @final
    def _create_block_bias_tensor(
        self,
        q_seq_ranges: list[tuple[int, int]],
        q_len: int,
        k_seq_ranges: list[tuple[int, int]],
        k_len: int,
        device: Device,
        dtype: DataType,
    ) -> Tensor:
        ...
        for (q_begin, q_end), (k_begin, k_end) in zip(q_seq_ranges, k_seq_ranges):
            if prev_q_end != q_begin and prev_k_end != k_begin:  # padding
