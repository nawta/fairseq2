...
class ShawRelativePositionSDPA(SDPA):
    ...
    def forward(
        ...
        *,
        ...
        if seqs_layout.is_packed or keys_layout.is_packed:
            raise NotSupportedError(
                f"`{ShawRelativePositionSDPA}` does not support packed batch layout."
            )
        ...
        rel_weights = torch.einsum("nhsk,stk->nhst", seqs, rel_keys)

        # We treat `rel_weights` as attention bias.
        rel_weights = rel_weights * (seqs.size(-1) ** -0.5)
