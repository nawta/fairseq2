...
class SDPA(Module, ABC):
    ...
    def _maybe_get_attention_bias_tensor(
        ...
        if isinstance(self.bias, CausalAttentionBias):
            if not seqs_layout.is_padded:
                seq_len = seqs_layout.shape[1]
                if seq_len == 1:
                    return None
