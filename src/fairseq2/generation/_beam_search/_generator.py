...
class BeamSearchSequenceGenerator(SequenceGenerator):
    ...
    def __call__(
        ...
        if prompt_seqs_layout.is_packed:
...
class BeamSearchSeq2SeqGenerator(Seq2SeqGenerator):
    ...
    def __call__(
        ...
        if source_seqs_layout.is_packed:
...
class _AbstractBeamSearchSequenceGeneratorOp(ABC):
    ...
    def __init__(
        ...
        prompt_seq_lens = prompt_seqs_layout.seq_lens

        self._min_prompt_len = max_seq_len
        self._max_prompt_len = 0

        for i, seq_len in enumerate(prompt_seq_lens):
            if seq_len == 0:
                raise ValueError(f"`prompt_seqs[{i}]` must not be empty.")

            if seq_len >= max_seq_len:
                raise ValueError(
                    f"The length of `prompt_seqs[{i}]` must be less than `max_seq_len` ({max_seq_len}), but is {seq_len} instead."
                )

            self._min_prompt_len = min(self._min_prompt_len, seq_len)
            self._max_prompt_len = max(self._max_prompt_len, seq_len)
        ...
        if prompt_seqs_layout.is_padded:
            # (P)
            self._prompt_lens = prompt_seqs_layout.seq_lens_pt

            # (P, S_prm)
            self._prompt_mask = prompt_seqs_layout.get_padding_mask()
        else:
