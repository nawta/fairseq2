...
from fairseq2.models.w2vbert import W2VBertLoss, W2VBertOutput
...
class W2VBertMetricBag(TaskMetricBag):
    """Holds the metrics of a w2v-BERT model training task."""
    ...
    def __init__(self, gang: Gang, train: bool = True) -> None:
        ...
        self.register_metric("_bert_loss", Mean(device=d), persistent=False)

        self.register_metric("_w2v2_loss", Mean(device=d), persistent=False)
    ...
    def update_losses(self, loss: W2VBertLoss, num_targets: int) -> None:
        """Update the loss metrics."""
        n = num_targets * math.log(2)

        self._loss.update(loss.total.detach() / n, weight=num_targets)

        self._bert_loss.update(loss.bert.detach() / n, weight=num_targets)

        self._w2v2_loss.update(loss.w2v2.total.detach() / n, weight=num_targets)

        self._contrastive_loss.update(
            loss.w2v2.contrastive.detach() / n, weight=num_targets
        )

        self._diversity_loss.update(
            loss.w2v2.diversity.detach() / n, weight=num_targets
        )

        self._feature_penalty.update(
            loss.w2v2.feature_penalty.detach() / n, weight=num_targets
        )

    @torch.inference_mode()
    def update_accuracy(self, output: W2VBertOutput) -> None:
        """Update the prediction accuracy."""
        # (N x S)
        predictions = output.w2v2_output.logits.argmax(-1).view(-1)
